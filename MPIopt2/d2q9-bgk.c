...
// 此代码优化了reduce,reduce好像其实一开始就没有计时！注意我们最好只关注那个迭代中的计算
// 这个代码没什么差别 可以忽略！
...
int main(int argc, char* argv[])
{
    ...
    /* ---- 新增：异步归约持久变量 ---- */
    MPI_Request red_req = MPI_REQUEST_NULL;   /* 上一轮 Ireduce 句柄        */
    double       global_u = 0.0;              /* 归约结果，仅 rank‑0 使用   */
    double       local_u  = 0.0;              /* 本轮局部 ∑|u|             */
    ...
    for(int tt = 0; tt < params.maxIters; tt++)
    {
        ...
        // 新增
        /* ---- 先收尾上一轮 Ireduce（从第 1 步开始才有） ---- */
        if (tt > 0) {
            MPI_Wait(&red_req, MPI_STATUS_IGNORE);           /* 等上轮网络完成      */
            if (ThisRank == 0)                               /* 只有 rank‑0 写出    */
                av_vels[tt-1] = (float)(global_u /
                                        (double)numberOfNonObstacles);
        }
        ...
        // 新增
        /* ---- (E) 发起本轮非阻塞归约，立刻返回 ---- */
        local_u = (double)(tot_u_in + tot_u_bd);
        MPI_Ireduce(&local_u, &global_u, 1, MPI_DOUBLE, MPI_SUM,
                    0, MPI_COMM_WORLD, &red_req);
    ...
    if (ThisRank == 0){
        av_vels[params.maxIters-1] =
                (float)(global_u / (double)numberOfNonObstacles);
    }
